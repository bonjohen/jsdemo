...
const AIGameController = ({
  ...
  // AI player
  const aiPlayerRef = useRef(new AIPlayer(aiDifficulty));
  ...
  const startGame = useCallback(() => {
    ...
    // Reset AI player
    aiPlayerRef.current = new AIPlayer(aiDifficulty);
    
    // Generate first pattern
    generateNewPattern();
  }, [generateNewPattern, aiDifficulty]);
  ...
  const handlePlayerTileClick = (index, selections) => {
    ...
    if (selections.length === currentPattern.length) {
      ...
      // Record player's result for AI learning
      ...
      setTimeout(() => {
        ...
        // AI attempts to memorize the pattern
